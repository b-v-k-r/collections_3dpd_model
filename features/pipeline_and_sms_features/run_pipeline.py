...
run_pipeline.py
===============
Executes all three SMS / in-app feature pipeline SQL files against Snowflake
in the correct order:

    1. pipeline_sms1.sql   – day-level SMS features (Collect DPD)
    2. pipeline_sms2.sql   – final SMS features (bank/CC/bounce windows)
    3. pipeline_inapp.sql  – in-app / transaction features
...
Usage
-----
    # Run all three pipelines with the default base table:
...
# ── SQL file order matters: sms1 creates the day-level table that sms2 reads ─
PIPELINE_DIR = Path(__file__).resolve().parent
SQL_FILES = [
    PIPELINE_DIR / "pipeline_sms1.sql",
    PIPELINE_DIR / "pipeline_sms2.sql",
    PIPELINE_DIR / "pipeline_inapp.sql",
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run SMS & in-app feature pipelines")
    parser.add_argument(
        "--base-tbl",
        default=None,
        help=(
    ...
    file_map = {
        "sms1": [PIPELINE_DIR / "pipeline_sms1.sql"],
        "sms2": [PIPELINE_DIR / "pipeline_sms2.sql"],
        "inapp": [PIPELINE_DIR / "pipeline_inapp.sql"],
        "all": SQL_FILES,
    }
